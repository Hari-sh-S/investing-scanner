...
class HuggingFaceManager:
    ...
    # ========== YFinance Data Methods ==========
    # These store YFinance data in 'yfinance/' subfolder, separate from Dhan data in 'data/'
    
    def _get_yfinance_symbol_path(self, symbol: str) -> str:
        """Get the path within the HF repo for a YFinance symbol's data file."""
        # Store with .NS suffix stripped for cleaner naming
        clean_symbol = symbol.replace('.NS', '').replace('.BO', '')
        return f"yfinance/{clean_symbol}.parquet"
    
    def download_yfinance_symbol(self, symbol: str) -> Optional[pd.DataFrame]:
        """Download YFinance data for a symbol from Hugging Face.
        ...
        try:
            file_path = hf_hub_download(
                repo_id=self.repo_id,
                filename=self._get_yfinance_symbol_path(symbol),
                repo_type="dataset",
                token=self.token
            )
            ...
            print(f"Error downloading YFinance {symbol} from HF: {e}")
            return None
    
    def upload_yfinance_symbol(self, symbol: str, df: pd.DataFrame) -> bool:
        """Upload YFinance data for a symbol to Hugging Face.
        ...
        try:
            ...
            upload_file(
                path_or_fileobj=tmp_path,
                path_in_repo=self._get_yfinance_symbol_path(symbol),
                repo_id=self.repo_id,
                repo_type="dataset",
                token=self.token
            )
            ...
            print(f"Error uploading YFinance {symbol} to HF: {e}")
            return False
    
    def get_yfinance_date_range(self, symbol: str) -> tuple[Optional[date], Optional[date]]:
        """Get the date range of existing YFinance data for a symbol.
        
        Returns:
            (min_date, max_date) or (None, None) if no data exists
        """
        df = self.download_yfinance_symbol(symbol)
    ...
    def get_yfinance_missing_dates(
        self, 
        symbol: str, 
        from_date: date, 
        to_date: date
    ) -> list[tuple[date, date]]:
        """Get date ranges that need to be fetched from YFinance.
        
        Returns list of (start, end) tuples representing gaps in data.
        """
        existing_min, existing_max = self.get_yfinance_date_range(symbol)
    ...
    def sync_yfinance_symbol(
        self,
        symbol: str,
        new_data: pd.DataFrame
    ) -> bool:
        """Sync YFinance data for a symbol - merge with existing and upload.
        ...
        # Upload merged data
        return self.upload_yfinance_symbol(symbol, combined)
    
    def list_yfinance_symbols(self) -> list[str]:
        """List all YFinance symbols available in the HF repository."""
        try:
            from huggingface_hub import list_repo_files
            ...
            # Extract symbol names from yfinance/ folder
            symbols = []
            for f in files:
                if f.startswith("yfinance/") and f.endswith(".parquet"):
                    symbol = f[9:-8]  # Remove "yfinance/" and ".parquet"
                    symbols.append(symbol)
